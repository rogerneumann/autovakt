# Vakt — Android Auto OBD2 Dashboard
...
Vakt registers as a `MediaBrowserService` to occupy the persistent 1/3 media slot in Android Auto (Coolwalk). The panel shows live telemetry instead of song info and stays visible while you navigate. Three swappable views, cycled with the skip buttons:
...
### Vakt Bridge
A local TCP server on port 35000 that emulates a WiFi ELM327 adapter. Apps like **ABRP**, **Torque Pro**, and **Car Scanner** can connect to it and get raw OBD2 data while Vakt holds the Bluetooth connection. No massaging — raw command → adapter → raw response.
...
- Demo mode (5-tap easter egg on VAKT title — no dongle needed)
...
**Bridge flow**: Third-party app → TCP port 35000 → `VaktBridgeServer` → `ElmCommandQueue` (same queue as Vakt) → raw response back to client.
...
2. The app registers two AA entry points:
   - **Full-screen dashboard** — find "Vakt" in the AA launcher (IOT category)
   - **Mini-player** — select Vakt as the active media source in the AA media picker
